...
# 既に import 済み: from sqlalchemy import text
...
def add_test_data():
    ...
    try:
        ...
        with engine.connect() as conn:
            print("\n=== customers テーブル ===")
            # 1) DESCRIBE 風の出力（SHOW COLUMNS）
            cols = conn.execute(text("SHOW COLUMNS FROM customers")).fetchall()
            print(tabulate(cols, headers=["Field","Type","Null","Key","Default","Extra"], tablefmt="psql"))

            # 2) データの出力
            rows = conn.execute(text("SELECT id, customer_id, customer_name, age, gender FROM customers")).fetchall()
            print(tabulate(rows, headers=["id","customer_id","customer_name","age","gender"], tablefmt="psql"))
            print("=========================\n")
